...
`tools/obj2bin.py` は、ユーザープログラムに `sample_asm_src/init.obj` と
`sample_asm_src/inout.obj` を自動リンクし、COMET II FPGA がそのまま読める
16-bit little-endian の `.bin` を生成します。

```sh
cd tools/sample_asm_src
python3 ../obj2bin.py hanoi.obj -o hanoi.bin
```
