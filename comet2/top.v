...
module top(input [3:0] SW, input clk, output LED_R, output LED_G, output LED_B,
   ...
   uart_mm #(.CLK_HZ(12000000), .BAUD(115200), .FIFO_DEPTH(16)) uart_mm_inst(
      .clk(clk), .reset(uart_reset),
   ...
   begin
      ...
      if(cpu_read_req_buf == 0 && cpu_read_req == 1) begin //only rising edge
         ...
         if(comet_mmio_uart) begin
            uart_rd_req <= 1;
            if(cpu_read_addr[3:0] == 4'h0) begin
               uart_rd_addr <= 32'h0;
            end else if(cpu_read_addr[3:0] == 4'h1) begin
               uart_rd_addr <= 32'h4;
            end else if(cpu_read_addr[3:0] == 4'h2) begin
               uart_rd_addr <= 32'h8;
            end else begin
               uart_rd_addr <= 32'hC;
            end
      ...
      if(cpu_write_req == 1) begin
         ...
         if(comet_write_mmio_uart) begin
            if(cpu_write_addr[3:0] == 4'h2) begin
               uart_wr_req <= 1;
               uart_wr_addr <= 32'h8;
               uart_wr_data <= {24'b0, cpu_write_data[7:0]};
            end else if(cpu_write_addr[3:0] == 4'h3) begin
               uart_wr_req <= 1;
               uart_wr_addr <= 32'hC;
