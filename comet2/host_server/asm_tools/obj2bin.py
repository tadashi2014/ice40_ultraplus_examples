...
FIRMWARE_DIR = TOOLS_DIR.parent / "firmware"
...
def parse_machine_words(field: str) -> ParsedCode:
    text = field.strip()
    if not text:
        return ParsedCode([])
    parts = text.split("-")
    ...
    for part in parts[:2]:
        ...
        if len(token) != 4 or any(ch not in "0123456789abcdefABCDEF" for ch in token):
            return ParsedCode(words, unresolved=token)
        words.append(int(token, 16))
    return ParsedCode(words)
...
def parse_linked_image(exe_path: Path) -> LinkedImage:
    ...
    with exe_path.open("r", encoding="utf-8") as fp:
        for raw in fp:
            ...
            parsed = parse_machine_words(code)
            if parsed.unresolved is not None:
                raise ValueError(
                    f"unresolved or non-hex machine word '{parsed.unresolved}' remains in {exe_path} at {addr:04X}. "
                    "Assemble linked modules with cross-reference information (-x) or pass .cas files to obj2bin."
                )
            for offset, word in enumerate(parsed.words):
