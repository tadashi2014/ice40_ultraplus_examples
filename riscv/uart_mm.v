...
module uart_mm #(
   ...
   always @(posedge clk) begin
      if (reset) begin
         ...
         if (buf_rd_req) begin
            data_valid <= 1'b1;
            if (buf_rd_addr == 32'h0) begin
               // bit1 reports "TX not ready for another byte yet" so simple
               // polling software can safely wait for this bit to clear before
               // each write without overrunning the FIFO.
               rd_data <= {28'h0, (tx_count == FIFO_DEPTH), rx_overflow, (tx_busy || tx_wr_en || (tx_count != 0)), (rx_count != 0)};
            end else if (buf_rd_addr == 32'h4) begin
               if (rx_count != 0) begin
            ...
            end else if (buf_rd_addr == 32'h8) begin
         ...
         if (wr_req) begin
            if (wr_addr == 32'h8) begin
               // Launch immediately when the transmitter is idle so bit1
               // behaves like a simple "busy" flag for polled writers.
               if (!tx_busy && !tx_wr_en && tx_count == 0) begin
                  tx_wr_en <= 1'b1;
                  tx_wr_data <= wr_data[7:0];
