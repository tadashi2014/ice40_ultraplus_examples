...
`ifdef CPU_COMET2
         if(cpu_read_addr[15:14] != 2'b11) begin
            memory_rd_req <= 1;
            memory_rd_addr <= cpu_read_addr;
         end
         if(cpu_read_addr[15:8] == 8'hC0) begin
            spi_rd_req <= 1;
         ...
         if(cpu_read_addr[15:8] == 8'hC1) begin
            gpio_rd_req <= 1;
            gpio_rd_addr <= 0;
         end
`else
...
`ifdef CPU_COMET2
         if(cpu_write_addr[15:14] != 2'b11) begin
            memory_wr_req <= 1;
            memory_wr_addr <= cpu_write_addr;
            memory_wr_data <= {16'b0, cpu_write_data[15:0]};
         end
         if(cpu_write_addr[15:8] == 8'hC0) begin
         ...
         if(cpu_write_addr[15:8] == 8'hC1) begin
            gpio_wr_req <= 1;
            gpio_wr_addr <= 0;
