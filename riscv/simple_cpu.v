   ...
   if(reset == 1) begin
      ...
      ID_BUF: begin //register file has 1clock delay, wait for the data read last state ....
         ...
         if(future_state == EX_S) begin
            read_1_en <= 1'b1;
            read_reg_1 <= instruction_sav[19:15]; //rs1: base address
            read_2_en <= 1'b1;
            read_reg_2 <= instruction_sav[24:20]; //rs2: store data
         end
      end
      EX_LD: begin
